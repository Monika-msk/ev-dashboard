...
export default function App() {
 ...
 useEffect(() => {
  ...
  }, [activeCorridor, searchQuery, showAllSites]);

  return (
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', backgroundColor: darkMode ? '#111' : '#fff', color: darkMode ? '#eee' : '#000' }}>
      <div style={{ width: '260px', padding: '12px', backgroundColor: darkMode ? '#222' : '#f4f4f4', overflowY: 'auto' }}>
        <h2>EV Corridors</h2>
        ...
        {corridorDefs.map(c => (
          <button
            ...
            style={{
              display: 'block', width: '100%', marginBottom: '8px', padding: '8px 12px',
              background: activeCorridor === c.id ? c.color : '#fff',
              color: activeCorridor === c.id ? '#fff' : '#333',
              border: `1px solid ${c.color}`, borderRadius: '4px', textAlign: 'left', cursor: 'pointer'
            }}
          >
            {c.id} - {c.name}
          </button>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', position: 'relative' }}>
        <div style={{ padding: '10px', borderBottom: '1px solid #ccc', background: darkMode ? '#333' : '#f4f4f4', color: darkMode ? '#eee' : '#000' }}>
          <input
            value={searchQuery}
            onChange={e => setSearchQuery(e.target.value)}
            placeholder="Search site..."
            style={{ padding: '10px', fontSize: '16px', borderRadius: '4px', border: '1px solid #ccc', marginRight: '10px' }}
          />
          <button onClick={() => setMapStyle(mapStyle.includes('streets') ? 'mapbox://styles/mapbox/satellite-v9' : 'mapbox://styles/mapbox/streets-v11')}>Toggle Map</button>
          <button onClick={() => { setShowAllSites(true); setShowTable(true); }}>Show All Sites</button>
        </div>
      ...
      {showTable && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: 'rgba(0,0,0,0.5)', zIndex: 999 }}>
          <div style={{ background: darkMode ? '#222' : '#fff', padding: '20px', borderRadius: '8px', maxHeight: '90vh', width: '95%', overflowY: 'auto', margin: '40px auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}>
              <h3>All EV Sites ({filteredSites.length})</h3>
              <button onClick={() => { setShowTable(false); setShowAllSites(false); }}>Close</button>
            </div>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr>{tableHeaders.map((h, i) => <th key={i} style={{ padding: '8px', border: '1px solid #ccc', backgroundColor: darkMode ? '#333' : '#eee' }}>{h}</th>)}</tr>
              </thead>
              <tbody>
                {filteredSites.map((site, i) => (
                  <tr key={i}>
                    <td style={{ border: '1px solid #ccc', padding: '8px' }}>{site.id}</td>
                    <td style={{ border: '1px solid #ccc', padding: '8px' }}>{site.corridor}</td>
                    <td style={{ border: '1px solid #ccc', padding: '8px' }}>{site.highway}</td>
                    <td style={{ border: '1px solid #ccc', padding: '8px' }}>{site.distanceFromHighway}</td>
                    <td style={{ border: '1px solid #ccc', padding: '8px' }}>{site.siteSize}</td>
                    <td style={{ border: '1px solid #ccc', padding: '8px' }}>{site.amenities}</td>
                    <td style={{ border: '1px solid #ccc', padding: '8px' }}>{site.substation}</td>
                    <td style={{ border: '1px solid #ccc', padding: '8px' }}>{site.renewables}</td>
                    <td style={{ border: '1px solid #ccc', padding: '8px' }}>{site.contact}</td>
                  </tr>
                ))}
              </tbody>
            </table>
